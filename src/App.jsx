...
function formatGrapes(grapes) {
  ...
  // alleen de druivensoorten zelf, geen verhoudingen.
  const clean = grapes.replace(/\d+([.,]\d+)?\s*%/g, "").replace(/\s{2,}/g, " ").trim();
  return clean.split(/\s*;\s*/).map(s => s.trim()).filter(Boolean).join(" • ");
}
...
function WineCard({ w, onOpen, onExpand, expanded, onMatchDish, onSetQuantity, onUpdateWine, onDelete, onCardCamera, onRequestScroll }) {
...
{"ratings": {"vivino": 4.2}}`;
    ...
    try {
      ...
      onRequestScroll && onRequestScroll(w.id);
    } catch (e) {
...
{"priceValue": "€56 - 65", "priceNote": "Wijnvoordeel.nl €60"}`;
  ...
  return (
    <>
    <div ref={cardRef} data-wine-id={w.id} className="card-scene" onClick={() => onExpand(w.id)}>
        ...
        <div className="card-face card-back">
          <div className="card-back-inner">
            ...
            <div className="back-header">
              ...
              <button className="refresh-btn back-edit-btn" onClick={openEditModal} title="Wijn wijzigen">
                <Pencil size={14} />
              </button>
            </div>
            ...
            <div className="back-row">
              {ar ? (
                ...
                <>
                  <div className="back-row-icon back-score-circle-empty" />
                  <div className="back-row-body">
      ...
      {showEditModal && editForm && (
        <div className="modal-overlay modal-overlay-center" onClick={(e) => { e.stopPropagation(); setShowEditModal(false); }}>
          <div className="modal add-choice-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span>Wijn wijzigen</span>
              <X size={20} className="modal-close" onClick={() => setShowEditModal(false)} />
            </div>
            <div className="edit-form-grid">
              <label className="edit-form-label">
                Naam
                <input value={editForm.wine} onChange={e => setEditForm({ ...editForm, wine: e.target.value })} />
              </label>
              <label className="edit-form-label">
                Wijnhuis
              ...
              <label className="edit-form-label">
                Jaargang
                <input value={editForm.vintage} onChange={e => setEditForm({ ...editForm, vintage: e.target.value })} />
              </label>
              <label className="edit-form-label">
                Regio / plaats
...
function PairingView({ wines, onOpen, onSetQuantity, onUpdateWine, onDelete, onCardCamera, onGoToWine, seedWineId, onClearSeed }) {
  ...
  if (seedWineId && seedWine) {
    return (
      <>
        ...
        <div className="seed-wine-banner" onClick={() => onGoToWine && onGoToWine(seedWine.id)}>
        ...
        {dishStatus === "done" && dishSuggestions.length > 0 && (
          <div className="pairing-results">
            ...
            <button
              className="btn-open more-suggestions-btn"
...
export default function CellarApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data && data.data) {
          let changed = false;
          const migrated = data.data.map(w => {
            const fix = TEXT_MIGRATION_V1[w.id];
            if (!fix) return w;
            const patch = {};
            if (fix.wine && fix.wine !== w.wine) patch.wine = fix.wine;
            if (fix.country && fix.country !== w.country) patch.country = fix.country;
            if (fix.region && fix.region !== w.region) patch.region = fix.region;
            if (fix.tastingNotes && fix.tastingNotes !== w.tastingNotes) patch.tastingNotes = fix.tastingNotes;
            if (fix.organic && fix.organic !== w.organic) patch.organic = fix.organic;
          ...
          setWines(migrated);
          if (changed) {
            await supabase.from("cellar").upsert({ id: CELLAR_ROW_ID, data: migrated, updated_at: new Date().toISOString() });
          }
        } else {
          setWines(SEED_DATA);
  ...
  useEffect(() => {
    ...
    // Wacht een tick zodat de (her)sortering en DOM al is bijgewerkt voordat we zoeken.
    const t = setTimeout(() => {
      const el = document.querySelector(`[data-wine-id="${scrollToId}"]`);
      if (el) {
        const stickyBar = document.querySelector(".cellar-header");
        const stickyHeight = stickyBar ? stickyBar.offsetHeight : 0;
        const rect = el.getBoundingClientRect();
        const targetTop = window.scrollY + rect.top - stickyHeight - 10;
        window.scrollTo({ top: Math.max(0, targetTop), behavior: "smooth" });
      }
      setScrollToId(null);
    }, 150);
  ...
  return (
    <div className="cellar-app">
      ...
      <style>{`
        ...
        .back-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; margin-bottom: 14px; }
        .back-edit-btn { flex-shrink: 0; margin-top: 2px; }
        ...
        .back-producer-bold { font-weight: 400; color: var(--ink); }
        .back-section-label {
        ...
        .organic-badge {
          ...
          flex-shrink: 0; color: #2F9B62; margin-top: 1px;
        }
        ...
        .more-suggestions-btn .photo-processing-spinner-sm {
        ...
        .seed-wine-banner {
          ...
          border-radius: 24px; margin-bottom: 4px; overflow: hidden; cursor: pointer;
          display: flex; align-items: stretch; justify-content: space-between; gap: 12px;
        }
        .seed-wine-text { min-width: 0; padding: 18px 0 18px 18px; flex: 1; display: flex; flex-direction: column; justify-content: center; }
        .seed-wine-name {
          font-family: 'Playfair Display', Georgia, serif;
        ...
        .seed-wine-photo {
          height: auto; width: 110px; object-fit: cover; flex-shrink: 0; display: block;
        }
        .dish-card {
        ...
        .edit-form-grid { display: flex; flex-direction: column; gap: 12px; margin-bottom: 8px; }
        .edit-form-label {
        ...
        .edit-form-label input {
          ...
          border: 1px solid var(--line); border-radius: 12px; padding: 10px 12px; font-size: 16px;
          background: var(--surface-solid); color: var(--ink);
        }
        .modal-divider {
